...
void idt_init(void) {
    ...
    // 2. Zero all 256 entries
    for (int i = 0; i < 256; i++) {
        idt_set_gate(i, 0, 0x08, 0);
    }

    // 3. Remap PIC — fill in the outb calls
    outb(0x20, 0x11);
    outb(0xA0, 0x11);
    outb(0x21, 0x20);
    outb(0xA1, 0x28);
    ...
    // 4. Register exception handlers 0-31
    ...
    // 6. Load IDT and enable interrupts
